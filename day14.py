...
# FIX 1 — Separate LLMs for separate jobs
query_llm = ChatGroq(
    api_key=os.environ.get("GROQ_API_KEY"),
    model_name="llama-3.1-8b-instant",
    temperature=0,      # deterministic query generation
    max_tokens=200
)
...
answer_llm = ChatGroq(
    api_key=os.environ.get("GROQ_API_KEY"),
    model_name="llama-3.1-8b-instant",
    temperature=0.1,    # slight flexibility for answer prose
    max_tokens=600
)
...
# ============================================================
RELEVANCE_THRESHOLD = 0.0  # FIX 3: reject negative scores
...
def hybrid_search(query, k_semantic=15, k_bm25=15, k_final=4):
    ...
    tokenized_query = tokenize(query)  # FIX 2: proper tokenization
    ...
    for doc in semantic_docs + bm25_docs:
        # FIX 4: use paper_id + chunk start for dedup
    ...
    # FIX 3 — Relevance floor: only return chunks above threshold
    reranked = []
    for score, doc in scored_docs[:k_final]:
        if score > RELEVANCE_THRESHOLD:  # reject negative scores
...
# ============================================================
# Step 6 — Compare semantic vs hybrid
# ============================================================
print("="*60)
...
def generate_queries(question):
    # Uses query_llm with temperature=0 — deterministic
...
# Contextualize uses query_llm (temperature=0)
...
def full_retriever(input_dict):
    ...
    return full_hybrid_retriever(question)

# Answer uses answer_llm (temperature=0.1)
rag_chain = (
...
# Run same question twice to verify determinism
